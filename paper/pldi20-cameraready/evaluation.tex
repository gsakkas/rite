...
We also observe that \trees achieves a Top-1 accuracy close 
to that of \dnn's (\ie \TreeTopOne\% vs. \DnnTopOne\%) 
but fails to improve with more predictions 
(\ie with Top-6, \TreeTopSix\% vs. \DnnTopSix\%).
%
On the other hand, the \svm does poorly one the Top-1 
accuracy (\ie \SVMTopOne vs. \DnnTopOne) but does 
significantly better with more predictions 
(\ie with Top-6, \SVMTopSix\% vs. \DnnTopSix\%).
%
Therefore, we observe that more sophisticated learning 
algorithms can actually learn patterns from a corpus of 
fixed programs, with \dnn classifiers achieving the best 
