...
In general, we find that humans perceive that
\toolname's fault localization and final repairs are both of higher quality
than those produced by \seminal in a statistically-significant manner ($p=0.030$
and $p=0.024$ respectively). All tests for
statistical significance used the Wilcoxon signed-rank test.

Regarding fault localization, we found that humans agreed with \toolname-identified 
edit locations 81.6\% of the time but agreed with those of \seminal only 74.0\% 
of the time. This 10\% increase is important because 
FIXME: You should explain why this matters. Also, consider
reordering these: first, show that we find the best location. Then, show
that our content is better as well. 

As for the synthesized repairs, humans also generally prefer \toolname's patches
...
of only 2.11/5, an increase of 14\% ($p=0.030$). As the inc 
...
useful for students, perhaps by citing something. Furthermore, this result
indicates that humans perceive \toolname's repairs to be better not only due to
better fault localization, but also due to the substitutive content of the
generated code.
...
To tease apart this quality assessment, we consider three case studies with 
statistically-significant differences between \toolname's and \seminal's
repairs. Figures~\ref{fig:riteBetterOne} and  
...
score of 4.1/5.}
\end{subfigure}
\caption{For each buggy student program, the underlined location indicates
the line modified for both patches.}
\label{fig:grammars}
\end{figure*}

\begin{figure}
  
  \caption{Example where humans found \toolname's repair of higher 
  quality than \seminal's repair (4.5 vs 1.0, $p=0.002$).
  repair by 1.4 ($p=0.028$). The goal of this program was to write
  some fun
  }
  \label{fig:riteBetterOne}
\end{figure}
