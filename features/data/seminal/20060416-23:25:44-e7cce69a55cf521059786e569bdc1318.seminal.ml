...
let rec subtype t1 t2 = (* ############################# *)
  ...
  | RecordT lst1,RecordT lst2 ->
	  let rec findmatch st ty ax lst =
		(match lst with
		  [] -> false
		| (st1,ty1,ax1)::tl ->
			  if ((st1 = st) && (subtype ty1 ty))  (* ######################################### *)
			  then true
			  else (findmatch st ty ax tl))
	  in (List.for_all (fun (s,t,a) -> findmatch s t a lst1) lst2)
  | _ -> false
...
let rec typecheck ctxt e = (* ########################### *)
  ...
  | If(e1,e2,e3) ->
      if (subtype e1 IntT)
      then (if (subtype e2 e3)
			then e3
			else (if (subtype e3 e2)
				  then e2
				  else raise (DoesNotTypecheck "if's e2/e3 not a subtype of eachother")))
      else raise (DoesNotTypecheck "if's e1 not a subtype of IntT")
  | RecordE lst ->
        raise Unimplemented
  ...
  | Cast(e,t) ->
	  checkType t;
	  if (subtype (typecheck ctxt e) t)
      then t
      else raise (DoesNotTypecheck "cast expression not a subtype of t")
