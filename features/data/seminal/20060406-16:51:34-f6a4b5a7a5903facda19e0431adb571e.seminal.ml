...
let print_prog exp = 
  let opt2str opt = match opt with
    None -> "<None>"
  | Some [] -> "<Empty list>"
  | Some lst -> List.fold_left (fun str cur -> str ^ ", " ^ cur) "" lst in
  let make_string_indent i s =
    (String.make i ' ') ^ s ^ "\n" in
  let rec pphelper exp indent =
    match exp with
      Var s -> make_string_indent indent ("Var " ^ s)
    | Lam(s, e, opt) -> 
        let expStr = pphelper e (indent+2) in
        (make_string_indent indent ("Lam(" ^ s ^ ",")) ^ 
        expStr ^ 
        (make_string_indent (indent+2) (opt2str opt)) ^
...
let _ = print_prog (fst (computeFreeVars ex1))
let _ = print_prog (fst (computeFreeVars ex2))

let mytest e = print_prog (fst (computeFreeVars e)); print_my_int (interp2 []
(fst(computeFreeVars e)))

(*let e = Second(Int(1))*)
(*let e = First(Int(1))*)
let e = First(Second(First(Pair(Pair(Pair(vx, vy), vx),vx),vx),vy),vx)
let _ = mytest e
