      ...
      ( let e2_type = typecheck ctxt e2 in
        let e3_type = typecheck ctxt e3 in
      	if (subtype e2_type e3_type) 
      	  then e3_type
      	  else (if (subtype e3_type e2_type) 
      	  	then e2_type
      	    	else (raise (DoesNotTypecheck ("If e2 and e3 are not " ^ 
      	    				      "sub/supertypes")))
      	       )
      )
      else raise (DoesNotTypecheck "If e1 is not subtype of IntT")
   | RecordV _ -> 
   	print_endline ("TC RecordV");
   	raise (DoesNotTypecheck "Not a source program")
   | RecordE lst -> 
   	print_endline ("TC RecordE");
   	let build_sta_lst (elt_str,elt_exp) = (elt_str, (typecheck ctxt elt_exp), Both) in
   	let mod_lst = (List.map build_sta_lst lst) in
   	fields_unique mod_lst; RecordT(mod_lst) 
   | Get(e,s) -> 
   	print_endline("TC Get "^ s);
   	(match (typecheck ctxt e) with
   	  RecordT lst-> let elt_info = getFieldType lst s in
   	    (match elt_info with
   	      None -> raise (DoesNotTypecheck ("Get record contains no field " ^s))
   	    | Some(s_type,s_access) -> 
   	    	if (s_access = Write) 
   	    	then raise (DoesNotTypecheck ("Get field "^s^" is Write-only"))
   	    	else s_type)
   	| _ -> raise (DoesNotTypecheck "Get expr is not Record type")  )
   | Set(e1,s,e2) ->
   	print_endline("TC Set " ^ s);
   	(match (typecheck ctxt e1) with
   	   RecordT lst -> let elt_info = getFieldType lst s in
   	   (match elt_info with
   	      None -> raise (DoesNotTypecheck ("Set record contains no field "^s))
   	    | Some(s_type,s_access) ->
   	    	if (s_access = Read)
   	    	then raise (DoesNotTypecheck ("Set field "^s^" is Read-only"))
   	    	else (if (subtype (typecheck ctxt e2) s_type)
   	    	      then s_type
   	    	      else raise (DoesNotTypecheck ("Cannot set "^s^" to e2 type"))
   	    	     )
   	    ) 
   	  | _ -> raise (DoesNotTypecheck "Set expr is not Record type"))
   	
   | Letrec(_,f,x,_,e) -> 
   	print_endline("TC Letrec "^f);
