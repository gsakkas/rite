...
let rec print_array array =
  match array with
    hd::tl -> print_endline (hd);print_array tl
  | []->()
...
let rec printExp e =
  ...
  | Lam(s,e2, vList) -> 
      (match vList with
	Some(lst) -> print_string ("Lam("^s^","))
  | Apply(exp1,exp2) -> print_string ("Apply(");printExp exp1;print_string (","); printExp exp2;print_string (")")
  | Closure(string, exp1, exp2) -> print_string ("Closure(");printExp exp1; print_string (",");printExp exp2; print_string (")")
...
print_array (snd (computeFreeVars ex1))
