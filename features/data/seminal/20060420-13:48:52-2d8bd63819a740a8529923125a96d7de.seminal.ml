...
let rec typecheck ctxt e = (* ########################### *)
  ...
  | Apply(e1,e2) -> print_endline ("TC Apply");
   ...
   | RecordV _ -> print_endline ("TC RecordV");
   	raise (DoesNotTypecheck "Not a source program")
   | RecordE lst -> 
   	let new_lst = List.map (typecheck ctxt) lst in
   	
   	if (fields_unique new_lst)
   	then
   	(
   	RecordT(new_lst)
   	)
   	else raise (DoesNotTypecheck "Duplicate field names in Record")
