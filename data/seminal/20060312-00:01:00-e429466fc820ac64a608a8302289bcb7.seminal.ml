...
type heapElement =  
      IntEntry of  int
    | HeapEntry of heap
      and heap = (string * heapElement) list
...
let rec lookup h str =
  ...
  |hd::tl -> 
      match hd with
        IntEntry (str1,i)->if str1=str then i else lookup h str
      | HeapEntry(str2,j)->if str2=str then j else lookup tl str

let update h str i = match i with
                        int -> IntEntry(str,i)::h
                     |	heap ->HeapEntry(str,i)::h			
