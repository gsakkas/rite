...
let rec computeFreeVars e = 
  ...
  | Lam(s,e2,opt) -> 
  	...
  	let test = (test_out s (add s lst)) in 
  	(match opt with
  	  None -> (Lam (s,e1, Some(add s lst)), add s lst)
  	| Some in_list -> (Lam(s,e1, Some(union (add s lst) in_list)),union (add s lst) in_list) )
  ...
  | Apply(e1,e2) -> print_endline("Apply");
  	...
  	let (exp2,lst2) = computeFreeVars e2 in
        let test = (test_out "Apply" (union lst1 lst2)) in
  	(Apply(exp1,exp2),Some(union lst1 lst2))
