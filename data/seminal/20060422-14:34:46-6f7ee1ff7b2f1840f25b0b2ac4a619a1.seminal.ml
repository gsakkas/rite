...
let rec typecheck ctxt e = (* ########################### *)
  ...
  | RecordV e -> 
      raise (DoesNotTypecheck "RecordV in source program")
  | RecordE lst ->
          (* ########################################################### *)
      let tlst = RecordT (List.map (fun (s,r) -> (s,(typecheck ctxt r)),Both) lst) in
      checkType tlst; tlst
  | Letrec(t1,_,_,t2,e) ->
  ...
  | Get(e, s) ->
          ...
          match typecheck ctxt e with
                RecordT rlst ->
                        getFieldType rlst s
              | _ -> raise (DoesNotTypecheck "non-RecordT in Get")
          ) in
          (
          match gettyp with
                None -> raise (DoesNotTypecheck "field not fpund inGet")
              | Some(_,Write) -> raise (DoesNotTypecheck "Write-only type in Get")
              | Some(t,_) -> t
          )
  | Set (e1,s,e2) ->
       let te1 = typecheck ctxt e1 in
       let te2 = typecheck ctxt e2 in
       if te1 <> te2 then raise (DoesNotTypecheck "e-types don't match in Set")
       else
          (
          let gettyp = 
          (
          match te1 with
                RecordT rlst ->
                        getFieldType rlst s
              | _ -> raise (DoesNotTypecheck "non-RecordT in Set")
          ) in
                  (
                  match gettyp with
                        None -> raise (DoesNotTypecheck "field not found in Set")
                      | Some(_,Read) -> raise (DoesNotTypecheck "Read-only type in Set")
                      | Some(t,_) -> t
                  )
          )
  | Cast(e,t) ->
          let etyp = typecheck ctxt e in
          if subtype etyp t then t 
          else raise (DoesNotTypecheck "invalid Cast")
