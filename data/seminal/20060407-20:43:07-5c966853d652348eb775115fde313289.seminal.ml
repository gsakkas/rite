...
print_endline ("huh");  match opt with
     None -> raise BadPrecomputation
   | Some lst -> List.map (fun s -> (s, List.assoc s env)) lst);print_endline ("huh2")
...
print_array (snd (computeFreeVars ex1)); printExp(fst (computeFreeVars ex1))
