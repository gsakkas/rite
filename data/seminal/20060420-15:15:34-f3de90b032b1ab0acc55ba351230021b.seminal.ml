...
let rec subtype t1 t2 = (* ############################# *)
  ...
  | RecordT(lt1),RecordT(lt2) ->
      List.for_all (
        fun (s2,rt2,a2) -> 
          match (getFieldType lt1 s2) with
            None -> false
          | Some(rt1,a1) ->
            let depthcheck = match a1,a2 with
...
let rec typecheck ctxt e = (* ########################### *)
  ...
  | RecordE(lt) -> 
      let t = RecordT(List.map (fun (s,e) -> (s,(typecheck ctxt e),Both)) lt) in
      checkType t; t 
  | RecordV _ -> raise (DoesNotTypecheck "")
  | Get(e,s) -> 
      (match (typecheck ctxt e) with
        RecordT(rt) -> 
         (match (getFieldType rt s) with
           None -> raise (DoesNotTypecheck "")
         | Some(t,a) -> if a = Write then raise (DoesNotTypecheck "") else t)
