...
let rec t2str t =
  ...
  | RecordT lst -> 
      "Rec[" ^ 
      (List.fold_left (fun cur (s,t,a) -> cur ^ "(\"" ^ s ^ "\", " ^ (t2str t) ^
...
let rec typecheck ctxt e = (* ########################### *)
  ...
  | Plus(e1,e2) ->
      ...
      then if subtype (typecheck ctxt e2) IntT
        then IntT
        else raise (DoesNotTypecheck ("Bad arg type in Plus. Expected IntT or
        subtype, got "
      ^ t2str (typecheck ctxt e2)))
      else raise (DoesNotTypecheck ("Bad arg type in Plus. Expected IntT or
      subtype, got "
      ^ t2str (typecheck ctxt e1)))
  | Apply(e1, e2) ->
      ...
      | _ -> print_type (typecheck ctxt e1); raise (DoesNotTypecheck ("Bad fun
type in Apply.  Expected ArrowT(_,_), got " ^ (t2str (typecheck ctxt e1)))))
  | If(e1, e2, e3) -> 
      ...
      else raise (DoesNotTypecheck ("Bad test type in If. Expected IntT or
      subtype, got " ^ (t2str (typecheck ctxt e1))))
  | RecordE(lst) -> 
  ...
  | Get(e,s) -> (match typecheck ctxt e with
    ...
    | _ -> raise(DoesNotTypecheck ("Bad record type in Get.  Expected RecordT _,
    got " ^ (t2str (typecheck ctxt e)))))
  | Set(e1,s,e2) -> (match typecheck ctxt e1 with
      RecordT lst -> (match getFieldType lst s with
          Some (t, Write) -> 
            if subtype (typecheck ctxt e2) t 
            then t 
            else raise (DoesNotTypecheck ("Bad rvalue type in Set(" ^ s ^ ").
            Expected " ^ (t2str t) ^ " or subtype, got " ^ (t2str (typecheck
            ctxt e2))))
        | Some (t, Both) -> 
            if subtype (typecheck ctxt e2) t 
            then t 
    ...
    | _ -> raise(DoesNotTypecheck ("Bad record type in Set.  Expected RecordT _,
    got " ^ (t2str (typecheck ctxt e1)))))
  | Letrec(tRet,sFunName,sArgName,tArg,eBody) -> 
      checkType tRet; checkType tArg;
      if subtype 
         (typecheck ((sArgName, tArg)::((sFunName,tRet)::ctxt)) eBody) 
         (ArrowT(tArg,tRet))
      then ArrowT(tArg, tRet)
      else raise (DoesNotTypecheck ("Bad fun body type in Letrec(" ^ sFunName ^
      ").  Expected " ^ (t2str (ArrowT(tArg, tRet))) ^ " or subtype, got " ^
      (t2str (typecheck ((sArgName, tArg)::((sFunName,tRet)::ctxt)) eBody))))
  | Cast(e,t) -> checkType t; if subtype (typecheck ctxt e) t then t else 
      raise (DoesNotTypecheck "Bad exp type in cast.  Expected " ^ (t2str t) ^ "
      or subtype, got " ^ (t2str (typecheck ctxt e)))
