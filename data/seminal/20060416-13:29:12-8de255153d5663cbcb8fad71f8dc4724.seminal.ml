...
let rec subtype t1 t2 = (* ############################# *)
  match t1,t2 with
      ...
      | RecordT fldlst1, RecordT fldlst2 -> 
              let rsubtype fld2 fld1 = subtype fld1 fld2 in
              let fldtst fld2 = List.exists (rsubtype fld2) fldlst1
              in List.for_all fldtst fldlst2 
      | RecordT fldlst1, IntT ->
              List.exists (fun x -> x = IntT) fldlst1
      | RecordT fldlst1, ArrowT(arg, res) ->
              List.exists (fun x -> x = ArrowT(arg, res)) fldlst1
      | _ -> false
