...
let histogram_fold acc a =
    match acc with
      Counter(i, c) ->
	let compChar = lowercase (char2str a) in 
	if (Char.compare (String.get compChar 0) c)=0 then
	  Counter(i+1,c)
	else
	  Counter(i, c)

let histogram_map str a =
  let arg = Counter(0, a) in
  fold_left histogram_fold arg str

let rec counter_to_intlist histData =
      match histData with
      [] -> []
    | hd::tl ->
	match hd with
	  Counter(i, c) ->
	    c::(counter_to_intlist tl)

let histogram s =
  let find_string = "abcdefghijklmnopqrstuvwxyz" in
  let histData = map (histogram_map s) find_string in
  counter_to_intlist histData
...
let _ = let arg1 = Sys.argv.(1) in 
  ...
  let output = printRec histData in
