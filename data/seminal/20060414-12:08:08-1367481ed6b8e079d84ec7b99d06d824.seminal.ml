...
let rec t2str t =
  ...
  | RecordT lst -> 
      ...
      "(\"" else cur ^ ", (\"") ^ s ^ "\", " ^ (t2str t) ^
      (match a with Read -> ", Read)" | Write -> ", Write)" | Both -> ", Both)")) "" lst) ^ 
      "]"
...
let ex1 = Cast(
        RecordE([("val", Cast(RecordE([("nest", Int(0)); ("nest2",Int(1))]),
                              RecordT([("nest",IntT,Both)])));
                 ("name", Int(0));
                 ("extra",Int(5))]), 
        RecordT([("name", IntT, Write); 
                 ("val",RecordT[("nest", IntT, Read)], Both)]))
let ex1 = If(Int(0), lam "y" IntT vy, lam "x" IntT vx)
let ex1 = If(Int(0), 
          lam "x" 
              (RecordT [("first", IntT, Read)]) 
              vx, 
          lam "y" 
              (RecordT [("first",IntT, Both);("second",ArrowT(IntT,IntT),Read)]) 
              (Cast(vy, RecordT([]))))

let sumN = Letrec(
               RecordT([("cur",IntT,Read);("acc",IntT,Read)]), 
               "loop",
               "x",
               IntT,
               (If(
                       ...
                       (app(
                               Var("loop"),
                               RecordE([("acc",
                               Plus(Get(vx,"cur"),Get(vx,"acc")));("cur",
                               Plus(Get(vx,"cur"),Int(-1)))]))),
                       Get(vx,"acc"))))
