...
let rec interpTrans movelist : float->float->float-> (float * float) list * float= 
     ...
     fun x y d -> 
       let compose2 f2 x1 y1 d1 lst1 =
         let _ = 
           ...
           | ((x2,y2)::tl2,d2) -> 
             print_endline ((string_of_int x1)^" "^(string_of_int y1)^" "^(string_of_int d1)^" "^(string_of_int x2)^" "^(string_of_int y2)^" "^(string_of_int d2))
         in
         match (f2 x1 y1 d1) with
         | (lst2, d2) -> (lst2@lst1, d2)
       in
       match (f1 x y d) with
       | ([], d1)            -> compose2 f2 x  y  d1 []
       | ((x1, y1)::tl1, d1) -> compose2 f2 x1 y1 d1 tl1
  in
  match movelist with
  | []                 -> fun x y d -> ([],d)
  | Home::tl           -> 
  ...
  | (For (i, lst)::tl) ->
     ...
     in
     compose (funfor i) (interpTrans tl)
