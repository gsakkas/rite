...
let rec subtype t1 t2 = (* ############################# *)
  ...
  | RecordT(l1),RecordT(l2) -> 
      (* ############################ *)
      let compareWidth = ((List.length l1) < (List.length l2)) in
      let compareTypes = 
        (let compareType le b = 
           let (s,t1a,_) = le in
           let st2a = getFieldType l2 s in
           match st2a with
           | None -> false
           | Some(t2a) -> b && (subtype t1a t2a) 
        in
        List.fold_right compareType l1 true)
      in
...
let _ = 
  print_endline (string_of_bool (subtype IntT IntT));
  print_endline (string_of_bool (subtype (ArrowT(IntT,IntT)) (ArrowT(IntT,IntT))));
  print_endline (string_of_bool (subtype (RecordT([])) (RecordT([]))));
  print_endline "Done"
