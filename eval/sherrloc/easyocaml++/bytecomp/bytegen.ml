...
let rec comp_expr env exp sz cont =
  ...
  match exp with
    Lvar id ->
      begin try
(*          Format.printf "Ident.find_same in stack %a %a@." Ident.print id (Ident.print_tbl Format.pp_print_int) env.ce_stack ;  *)
        let pos = Ident.find_same id env.ce_stack in
        Kacc(sz - pos) :: cont
      with Not_found ->
      try
(*          Format.printf "Ident.find_same in heap %a %a@." Ident.print id (Ident.print_tbl Format.pp_print_int) env.ce_heap ;  *)
        let pos = Ident.find_same id env.ce_heap in
        Kenvacc(pos) :: cont
      with Not_found ->
      try
(*          Format.printf "Ident.find_same in rec %a %a@." Ident.print id (Ident.print_tbl Format.pp_print_int) env.ce_rec ;  *)
